...
// lags the runtime at 1.4.21 and emits neither the `index(...)` callbacks nor the
// `rate_limit` table, so re-add both by hand after regenerating: every foreign
// key and every column better-auth filters on is indexed here, not by the
// generator.
