...
const cellClasses =
  "w-[1ch] h-[calc(var(--theme-line-height-base)*1rem)] bg-[var(--theme-border-subdued)] text-center align-middle";
const squareClasses =
  "h-[calc(var(--theme-line-height-base)*2rem)] text-center align-middle w-[3ch]";
const symbolClasses =
  "text-[40px] leading-[calc(var(--theme-line-height-base)*2rem)]";
...
const Chessboard = ({ board }: ChessboardProps) => {
  return (
    <table className="inline-table border-collapse">
      <tbody>
        <tr>
          <td className="h-[calc(var(--theme-line-height-base)*1rem)] w-[1ch]"></td>
          {FILE.map((file) => (
        ...
        {RANK.map((rank, rowIndex) => (
          <tr key={rank}>
            ...
            {FILE.map((_, colIndex) => {
              ...
              const squareClass = isDark
                ? "bg-[var(--theme-focused-foreground)]"
                : "bg-[var(--theme-focused-foreground-subdued)]";
              return (
