...
const TreeView: React.FC<TreeViewProps> = ({ defaultValue = false, title, children, depth = 0, isFile = false, isRoot = false, isLastChild = false, style, parentLines = [] }) => {
  ...
  return (
    <div className="whitespace-nowrap text-[100%]" style={style}>
      <div tabIndex={0} role="button" onClick={onToggleShow} className="cursor-pointer focus:outline-0 focus:border-0 focus:bg-[var(--theme-focused-foreground)]" aria-expanded={show}>
        {prefix}
        {icon}
        {title}
      </div>
