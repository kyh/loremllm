...
import { _DIRS, _filesHere } from "./fs";

export function runRootTerminal(term: ExtendedTerminal) {
        ...
        switch (e) {
          case "\r": {
            ...
            if (term.currentLine.length > 0) {
              ...
              const exitStatus = term.command(term.currentLine);

              if (typeof window !== "undefined") {
                const dataLayer = (window as { dataLayer?: unknown[] })
                  .dataLayer;
                if (dataLayer) {
                  dataLayer.push({
                    event: "commandSent",
          ...
          case "\x1b[A": {
            ...
            const historyReversed = [...term.history].reverse();
            if (term.historyCursor < historyReversed.length - 1) {
              term.historyCursor += 1;
              const historyLine = historyReversed[term.historyCursor];
              if (historyLine !== undefined) {
                term.setCurrentLine(historyLine, false);
              }
            }
            break;
          }
          case "\x1b[B": {
            ...
            const historyReversedDown = [...term.history].reverse();
            if (term.historyCursor > 0) {
              term.historyCursor -= 1;
              const historyLine = historyReversedDown[term.historyCursor];
              if (historyLine !== undefined) {
                term.setCurrentLine(historyLine, false);
              }
            } else {
          ...
          case "\t": {
            ...
            if (term.tabBase !== term.currentLine) {
              ...
              if (tabParts.length === 1) {
                ...
                term.tabOptions = _filesHere(term.cwd)
                ...
                term.tabOptions = _filesHere(term.cwd)
                  .filter(
                    (dir) =>
                      dir.startsWith(tabRest) &&
                      !_DIRS[term.cwd]?.includes(dir),
                  )
                  .sort();
              }
            }
